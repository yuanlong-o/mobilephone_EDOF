...
%% updating 
for miter=1:maxIter
    fprintf('start HALS iteration %d / %d in total, ', miter, maxIter);
    for k=1:K
        tmp_ind = active_pixel(:, k); 
        if any(tmp_ind)
            ak = max(0, A(tmp_ind, k)+(U(tmp_ind, k)-A(tmp_ind,:)*V(:, k))/cc(k)); % gradient + projection step
            A(tmp_ind, k) = ak; 
        end
    end
    error = sum((Y-A*C).^2, 'all');
    fprintf('error: %.6f \n', error);
end