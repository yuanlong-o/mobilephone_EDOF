...
if flag_AY
    ...
    U = mm_fun(A,Y); % here mm_fun is just a matrix multiplication
end
...
while repeat
    ...
    for k=1:K % for each component
        ck = C(k, :) + (U(k, :)-V(k, :)*C)/aa(k); % this is the gradient step
        if nonneg
            ck = max(0, ck);     % update ck, this is the projection step
        end
        C(k, :) = ck; 
    end
    repeat = (miter < maxIter) && norm(C - C_,'fro')/norm(C_,'fro') > tol; % reach the stop criteria
    miter = miter + 1;
end